...
class TestDangerousModule:
    ...
    def test_dangerous_module_creation(self):
    ...
    def test_module_with_different_severities(self):
        """Test modules with different severity levels."""
        modules = [
            DangerousModule("requests", "HTTP library", SecurityIssueSeverity.LOW),
            DangerousModule("socket", "Network operations", SecurityIssueSeverity.MEDIUM),
            DangerousModule("subprocess", "Process execution", SecurityIssueSeverity.HIGH),
        ]
...
class TestSecurityPolicy:
    ...
    def test_policy_with_patterns_and_modules(self):
    ...
    def test_add_pattern_to_policy(self):
    ...
    def test_add_dangerous_module_to_policy(self):
...
class TestSecurityPolicyIntegration:
    ...
    def test_policy_serialization_compatibility(self, sample_policy):
    ...
    def test_policy_modification_chain(self, sample_policy):
    ...
    def test_severity_filtering(self, sample_policy):
        """Test filtering patterns by severity level."""
        high_severity_patterns = [
            p for p in sample_policy.patterns 
            if p.severity >= SecurityIssueSeverity.HIGH
        ]
        
        medium_and_above_patterns = [
            p for p in sample_policy.patterns 
            if p.severity >= SecurityIssueSeverity.MEDIUM
        ]
    ...
    def test_module_severity_filtering(self, sample_policy):
        """Test filtering modules by severity level."""
        high_severity_modules = [
            m for m in sample_policy.dangerous_modules 
            if m.severity >= SecurityIssueSeverity.HIGH
        ]
...
class TestPatternMatching:
    ...
    def test_os_system_pattern_matching(self):
        ...
        for case in positive_cases:
            assert compiled_pattern.search(case) is not None, f"Should match: {case}"
        
        for case in negative_cases:
            assert compiled_pattern.search(case) is None, f"Should not match: {case}"
    ...
    def test_import_pattern_matching(self):
        ...
        import_pattern = r"(?:^|\s)(?:import\s+os(?:\s+as\s+\w+)?|from\s+os\s+import\s+(?:\*|\w+(?:\s+as\s+\w+)?(?:,\s*\w+(?:\s+as\s+\w+)?)*))(?=[\s;(#]|$)"
