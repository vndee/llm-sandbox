...
- **Container runtime** (at least one of the following):
    ...
    - Podman
    - Conda/Mamba for Micromamba backend
...
LLM Sandbox can automatically capture plots generated by your code:
...
```python
def execute_llm_code(code: str, language: str = "python"):
...
result = execute_llm_code(llm_generated_code)
print(result["output"])
```
...
with SandboxSession(
    lang="python",
    runtime_configs={"timeout": 10}  # 10 second timeout
) as session:
    result = session.run("import time; time.sleep(20)")  # Will timeout
```

### 4. Use Security Policies

Always use security policies when running untrusted code:
...
```python
# Keep template for reuse
with SandboxSession(lang="python", keep_template=True) as session:
    # Image will be kept for faster subsequent runs
    pass

# Or ensure cleanup
with SandboxSession(lang="python", keep_template=False) as session:
    # Image will be removed after use
    pass
```
