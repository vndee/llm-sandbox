...
def run_java_code() -> None:
    ...
    with SandboxSession(
        lang="java", keep_template=True, verbose=True, backend=SandboxBackend.PODMAN, client=client
    ) as session:
        output = session.run(
            """
            public class Main {
...
def run_cpp_code() -> None:
    ...
    with SandboxSession(
        lang="cpp", keep_template=True, verbose=True, backend=SandboxBackend.PODMAN, client=client
    ) as session:
        output = session.run(
            """
            #include <iostream>
...
def run_go_code() -> None:
    ...
    with SandboxSession(
        lang="go", keep_template=True, verbose=True, backend=SandboxBackend.PODMAN, client=client
    ) as session:
        output = session.run(
            """
            package main
